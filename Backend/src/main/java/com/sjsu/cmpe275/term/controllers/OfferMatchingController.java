...
public class OfferMatchingController {
	...
	public ResponseEntity<OfferMatchingDTO> postOffer(@PathVariable Long id) {
		try {
	        ...
	        List<Offer> matchedOffers = offerMatchingService.getSingleMatchesByID(offer.getId(), offer.getUserId(), todayDate, min , max );
			List<OfferDto> matchedOffersDtos = matchedOffers
					  .stream()
